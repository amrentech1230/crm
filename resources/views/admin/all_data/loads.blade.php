...
<tr class="load-row {{ 
    ...
    <td class="hide_blur_privacy"><a href="{{ route('load.edit', $load->id) }}">{{ $load->load_number }}</a></td>
    <td class="hide_blur_privacy">
        <a href="{{ route('load.edit', $load->id) }}" class="btn btn-primary btn-sm">
            <i class="fas fa-edit"></i>
        </a>
        <a href="{{ route('accounts.view_loads_detail', $load->id) }}" class="btn btn-primary btn-sm"> <i class="fas fa-eye"></i></a>
    </td>

    <td class="hide_blur_privacy">@if($load->user){{ $load->user->name }}@endif</td>
    <td class="hide_blur_privacy">
...
<td class="hide_blur_privacy">
    ...
    @elseif(!empty($load->invoice_status_date))
        {{ \Carbon\Carbon::parse($load->invoice_status_date)->format('m-d-Y') }}
    @else
        -
    @endif
</td>

    <td class="hide_blur_privacy">{{ $load->load_workorder }}</td>
    <td class="hide_blur_privacy">{{ $load->load_bill_to }}</td>
    <td class="hide_blur_privacy">@if($load->user){{ $load->user->officedata?->office_name }}@endif</td>
    <td class="hide_blur_privacy">@if($load->user){{ $load->user->teamLeaderInfo?->tl }}@endif</td>
    <td class="hide_blur_privacy">@if($load->user){{ $load->user->managerInfo?->manager }}@endif</td>
    <td class="hide_blur_privacy">{{ $load->created_at->format('m-d-Y') }}</td>
        @php
        $shipper_appointment = json_decode($load->load_shipper_appointment,true);
        @endphp
    <td class="hide_blur_privacy">{{ isset($shipper_appointment[0]['appointment']) ? \Carbon\Carbon::parse($shipper_appointment[0]['appointment'])->format('m-d-Y') : '' }}</td>
        @php
    ...
    <td class="hide_blur_privacy">{{ $load->load_mc_no }}</td>
    <td class="hide_blur_privacy">
 ...
 <td class="dynamic-data" style="vertical-align: middle !important;">${{ $load->shipper_load_final_rate }}</td>
<td class="dynamic-data" style="vertical-align: middle !important;">${{ $load->load_final_carrier_fee }}</td>
...
@endphp
<td class="dynamic-data" style="vertical-align: middle !important;">{{ number_format($marginPercent, 2) }}%</td>
    <td class="hide_blur_privacy">
        @php
