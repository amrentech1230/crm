@php
                                                $i = 1;
                                            @endphp
                                                @foreach($accounts as $getaccount)
                                                <tr>
                                                    <td class="dynamic-data">{{ $i++ }}</td>
                                                    <td>
                                                        <label class="switch">
                                                            <input class="switch-input statusCheckbox" type="checkbox" data-id="{{$getaccount->id}}" @if($getaccount->status == 'active') checked @endif>
                                                            <span class="switch-button"></span>
                                                        </label>
                                                    </td>
                                                    <td class="dynamic-data">{{ $getaccount->name }}</td>
                                                    <td class="dynamic-data">{{ $getaccount->email }}</td>
                                                    <td class="dynamic-data">{{ $getaccount->managers?->manager }}</td>
                                                    <td class="dynamic-data">{{ $getaccount->teamleader?->tl }}</td>
                                                    <td class="dynamic-data">{{ $getaccount->role?->name }}</td>
                                                    <td class="dynamic-data" style="text-align: center;">
                                                        ...
                                                        <button class="btn update-password-btn" data-bs-toggle="modal" data-bs-target="#updatePasswordModal{{ $getaccount->id }}" title="Change Password" data-id="{{ $getaccount->id }}" data-name="{{ $getaccount->name }}"><i class="fa fa-lock" style="color: #ff9d20;"></i></button>
                                                        <a href="{{route('delete_user',$getaccount->id)}}" class="deleteAccount" data-id="{{ $getaccount->id }}"><i class="fa fa-trash" style="color:red;font-size: 17px;cursor: pointer;"></i></a>
                                                    </td>
                                                </tr>
                                                <div class="modal fade" id="updatePasswordModal{{ $getaccount->id }}" tabindex="-1" aria-labelledby="updatePasswordModalLabel" aria-hidden="true">
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            <form class="updatePasswordForm" method="POST" action="{{route('admin_update_password', $getaccount->id)}}">
                                                                ...
                                                                <div class="modal-body">
                                                                    ...
                                                                    <div class="mb-3">
                                                                        ...
                                                                        <input type="text" class="form-control accountName" value="{{ $getaccount->name }}" readonly>
                                                                    </div>
                                                                    <div class="mb-3">
