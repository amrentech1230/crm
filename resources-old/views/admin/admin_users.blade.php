                                        ...
                                        <thead>
                                            ...
                                            @endphp
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <label class="switch">
                                                            <input class="switch-input statusCheckbox" type="checkbox" data-id="{{$getadmin->id}}" @if($getadmin->status == 'active') checked @endif>
                                                            <span class="switch-button"></span>
                                                        </label>
                                                    </td>
                                                    <td class="dynamic-data">{{ $getadmin->emp_code }}</td>
                                                    <td class="dynamic-data">{{ $getadmin->name }}</td>
                                                    <td class="dynamic-data">{{ $getadmin->email }}</td>
                                                    <td class="dynamic-data">**********</td> <!-- Hide password -->
                                                    <td class="dynamic-data">{{ $getadmin->address }}</td>
                                                    <td class="dynamic-data">{{ $getadmin->managers?->manager }}</td>
                                                    <td class="dynamic-data">{{ $getadmin->teamleader?->tl }}</td>
                                                    <td class="dynamic-data">{{ $getadmin->role?->name }}</td>
                                                    <td class="dynamic-data">{{ $getadmin->emergency_contact }}</td>
                                                <td class="dynamic-data">{{ $getadmin->created_at ? \Carbon\Carbon::parse($getadmin->created_at)->format('m-d-y') : '' }}</td>
                                                    <td class="dynamic-data" style="text-align: center;">
                                                        <!-- <a data-toggle="modal" style="margin-right:7px;" data-target="#editAccountModal" data-id="{{ $getadmin->id }}" class="editAccount"><i class="fa fa-edit" style="color:#0DCAF0;font-size: 17px;margin-left: 13px;cursor: pointer;"></i></a> -->
                                                        <button class="btn update-password-btn" data-bs-toggle="modal" data-bs-target="#updatePasswordModal" title="Change Password" data-id="{{ $getadmin->id }}" data-name="{{ $getadmin->name }}"><i class="fa fa-lock" style="color: #ff9d20;"></i></button>
                                                        <a href="javascript:void(0);" class="deleteAccount" data-id="{{ $getadmin->id }}"><i class="fa fa-trash" style="color:red;font-size: 17px;cursor: pointer;"></i></a>
                                                    </td>
                                                </tr>
                                                @endforeach
                                            </tbody>
